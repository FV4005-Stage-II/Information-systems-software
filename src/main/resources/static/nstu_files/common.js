...
$(function () {			
    ...
    }).on('loadMoreJobOffers', function (e, params) {
        $.getJSON(params.url+'&'+params.data, function (result) {
            params.callback();
            params.callbackSuccess(result);
        }, 'json');
    }).on('loadMoreActivities', function (e, params) {
        $.getJSON(params.url+'&'+params.data, function (result) {
            params.callback();
            params.callbackSuccess(result);
        }, 'json');
	...
	(function ($sliders) {
		...
		$sliders.each(function () {
			var $slider = $(this);	 
			var $slides = $('.js-slides', $slider);
			var customControl = (typeof $slider.data('customControl') === 'undefined' ) ? false : ($slider.data('customControl'));
			if ($slides.length) {
				var slidesToShow = (typeof $slider.data('toshow') === 'undefined' ) ? 1 : Number($slider.data('toshow'));	
				var slidesToScroll = (typeof $slider.data('toscroll') === 'undefined' ) ? 1 : Number($slider.data('toscroll'));
				if (customControl !== false) {
					var $status =  $($slider.data('customControl')).find('.slider__control-counter');
					$slides.on('init reInit afterChange', function (event, slick, currentSlide, nextSlide) {						
						if (slick.slideCount > slidesToShow ) {
							var i = (currentSlide ? currentSlide/slidesToScroll : 0) + 1;
							$status.text(i + '/' + ( Math.round(slick.slideCount/slidesToScroll) + slick.slideCount % slidesToScroll) );
							$status.removeClass('hidden');
						}
						else $status.addClass('hidden');
					});	
				}
				$slides.slick({
					infinite: (typeof $slider.data('infinite') === 'undefined' ) ? true : Boolean($slider.data('infinite')),
					autoplay: (typeof $slider.data('autoplay') === 'undefined' ) ? true : Boolean($slider.data('autoplay')),
					slidesToShow: slidesToShow,
					slidesToScroll: slidesToScroll,
					dots:  (typeof $slider.data('dots') === 'undefined' ) ? false : Boolean($slider.data('dots')),
					arrows: (typeof $slider.data('arrows') === 'undefined' ) ? false : Boolean($slider.data('arrows')),
					rows: 0,
					autoplaySpeed: (typeof $slider.data('speed') === 'undefined' ) ? 3000 : Number($slider.data('speed')),
					adaptiveHeight: true,
					prevArrow: 	(customControl === false) ? false : $($slider.data('customControl')).find('.slider__control-prev'),
					nextArrow: (customControl === false) ? false : $($slider.data('customControl')).find('.slider__control-next'),
					responsive: [
						{
							breakpoint: 1000,
							settings: {
								slidesToShow: (typeof $slider.data('mobile') === 'undefined' ) ? 1 : Number($slider.data('mobile')),
								slidesToScroll: (typeof $slider.data('mobile') === 'undefined' ) ? 1 : Number($slider.data('mobile')),
								infinite: true
							}
						}
					]
	...
	(function ($scopeElement) { 
        ...
        var $eventName = $scopeElement.data('event');
        var $baseUrl = $scopeElement.data('baseUrl');
        var $document = $(document);
        var $form = $('.js-selectors-form', $scopeElement);
        var $search = $('.js-selectors-search', $scopeElement);
        var $inputs = $('.js-selectors-input', $scopeElement);
        var $selects = $('.js-selectors-select', $scopeElement);
        var $more = $('.load-more', $scopeElement);
		
		$more.on('click', function() {
			loadData();
		});

		$search.bindWithDelay('input', function (e) {
			$more.data('url', $baseUrl+'?next=1');
			loadData("clear");
        }, 300);

		$selects.bindWithDelay('change', function () { 
			$more.data('url', $baseUrl+'?next=1');
			loadData("clear");
		}, 300);
        ...
        $inputs.on('change', function () { 
            var $this = $(this);
            var $parents = $this.parents('.js-selectors-filter-item');
            if($this.val() === 'all') {
                $parents.find('input:checked').not('[value="all"]').prop('checked', false);
            } else { 
                $parents.find('input[value="all"]').prop('checked', false);
            }
			if (!$parents.find('input:checked').length)
				 $parents.find('input[value="all"]').prop('checked', true);
			$more.data('url', $baseUrl+'?next=1');
            loadData("clear");
        });

        $('.js-selectors-filter-label', $scopeElement).on('click', function (e) { 
            e.preventDefault();
            var $this = $(this);
            var $parent = $this.parents('.js-selectors-filter-item'); 
            if($parent.hasClass('is-open')) {
                $parent.removeClass('is-open')
            } else {
                $parent.addClass('is-open')
        ...
        function loadData(param) {
            ...
            $document.trigger($eventName, {
                ...
                callbackSuccess: function (result) { 
                    if (Boolean(result.haveMore-0) === true) {
                        $more.data('url', result.nextUrl).show();
						
                    } else {
                        $more.hide();
                    }
                    if (result.items && result.items.length) {
						if (param == "clear") $target.html(result.items);
						else $(result.items).appendTo($target);
                    }
					else if (param=="clear") $target.html("");
                }
            });
